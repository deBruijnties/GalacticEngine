...
//      | ------------------| ------------------------------------------|
//      | `App`             | Window + main loop                        |
//      | `VulkanContext`   | Instance, surface, device, queues         |
//      | `Swapchain`       | Swapchain + image views + framebuffers    |
//      | `Pipeline`        | Render pass + graphics pipeline           |
//      | `Renderer`        | Command buffers + sync + drawFrame        |
...
VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& presentModes) {
    for (const auto& availablePresentMode : presentModes) {
        if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
            return availablePresentMode; // Low latency if available
        }
    }
